...
if (!codeConnectionArn) {
  ...
  new FrontendStack(app, `RNKbdDocsFrontend-${environment}`, {
    env: { account, region },
    environment,
    buildOutputPath,
...
// Pipeline stack (when codeConnectionArn is provided)
if (codeConnectionArn) {
  new PipelineStack(app, "RNKbdDocsPipelineStack", {
    env: { account, region },
    description: "CI/CD Pipeline for RNKbdDocs",
    codeConnectionArn,
    repositoryName,
    branchName,
...
cdk.Tags.of(app).add("Project", "RNKbdDocs");
